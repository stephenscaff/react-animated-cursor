...
function AnimatedCursor({
  ...
  useEffect(() => {
    if (isActiveClickable) {
      cursorInnerRef.current.style.transform = `translateZ(0) scale(${innerScale * 1.3})`
      cursorOuterRef.current.style.transform = `translateZ(0) scale(${outerScale * 1.4})`
    }
